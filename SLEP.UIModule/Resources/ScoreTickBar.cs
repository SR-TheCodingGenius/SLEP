...
namespace SLEP.UIModule.Resources
{
	...
	public class ScoreTickBar : TickBar
	{
		protected override void OnRender(DrawingContext dc)
		{
			try
			{
				...
				for (var count = 0; count <= tickCount; count++)
				{
					...
					{
						drawAtX = (tickFrequencySize * count) + 12;
					}
					else
					{
					...
					if (SliderContent.Comment.Count != 0 && SliderContent.Score.Count != 0)
					{
						...
						formattedText = new FormattedText(SliderContent.Score[count], CultureInfo.GetCultureInfo("en-us"), FlowDirection.LeftToRight, new Typeface("Helvetica Neue - thin"), 12, Brushes.Black);
						dc.PushTransform(new RotateTransform(90, drawAtX, -drawAtY));
						dc.DrawText(formattedText, new Point(drawAtX, -drawAtY));
						dc.Pop();
					}
